...
- **2019-02 · Adapter（参数高效迁移学习）** — Houlsby et al., ICML 2019. [arXiv:1902.00751](https://arxiv.org/abs/1902.00751) — 在每个 Transformer block 中插入瓶颈 MLP（bottleneck adapter），冻结 BERT 主体仅训练 adapter，首次系统验证"少量插入层即可接近全参数微调"的 PEFT 范式。

- **2021-01 · Prefix Tuning（前缀调优）** — Li & Liang, ACL 2021. [arXiv:2101.00190](https://arxiv.org/abs/2101.00190) — 在每层 attention 的 K/V 前拼接可学习前缀向量（通过 MLP reparameterize 保证训练稳定），只需 0.1% 参数即在生成任务上接近全量微调效果。

- **2021-04 · Prompt Tuning（软提示调优）** — Lester et al., EMNLP 2021. [arXiv:2104.08691](https://arxiv.org/abs/2104.08691) — 仅在输入层拼接可学习 soft token，冻结整个模型；发现模型规模足够大时软提示调优可与全参数微调持平，参数量极小。

- **2021-06 · LoRA（低秩适配）** — Hu et al., ICLR 2022. [arXiv:2106.09685](https://arxiv.org/abs/2106.09685) — 假设权重更新 ΔW 具有低秩结构，用 B、A 两个低秩矩阵参数化 ΔW = BA，推理时可 merge 回 base model 实现零额外延迟，成为此后所有 LoRA 系变体的基础。

- **2022-05 · (IA)³（激活缩放适配）** — Liu et al., NeurIPS 2022. [arXiv:2205.05638](https://arxiv.org/abs/2205.05638) — 对 attention 的 key/value 和 FFN 输出各乘以一个可学习缩放向量，参数量极少（每层仅数千），适合 few-shot 场景，merge 后零额外延迟。

- **2023-03 · AdaLoRA（自适应秩分配）** — Zhang et al., ICLR 2023. [arXiv:2303.10512](https://arxiv.org/abs/2303.10512) — 将 ΔW 参数化为类 SVD 形式 PΛQ，通过 importance score（奇异值大小×梯度灵敏度）动态剪枝低重要性奇异值，在固定参数预算下将更多 rank 分配给关键层。

- **2023-05 · QLoRA（4-bit 量化 + LoRA）** — Dettmers et al., NeurIPS 2023. [arXiv:2305.14314](https://arxiv.org/abs/2305.14314) — 将 base model 量化为 4-bit NF4 格式，LoRA adapter 保持 BF16 精度训练，配合 Double Quantization 和 Paged Optimizer，首次实现单卡训练 65B 模型。

- **2023-10 · VeRA（向量随机矩阵适配）** — Kopiczko et al., ICLR 2024. [arXiv:2310.11454](https://arxiv.org/abs/2310.11454) — 所有层共享同一套随机固定矩阵 A、B，仅训练逐层对角缩放向量 b 和 d，将可训练参数量压缩至 LoRA 的 1/k，适合极端参数受限场景。

- **2024-02 · DoRA（幅度-方向分解适配）** — Liu et al., ICML 2024. [arXiv:2402.09353](https://arxiv.org/abs/2402.09353) — 将权重更新分解为幅度（可学习向量 m）和方向（LoRA 低秩更新），使梯度行为更接近全参数微调，仅增加极少额外参数。

- **2024-02 · LoRA+（解耦学习率）** — Hayou et al., ICML 2024. [arXiv:2402.12354](https://arxiv.org/abs/2402.12354) — 基于 muP 理论分析证明 B 矩阵应使用比 A 矩阵更高的学习率（推荐比值 λ∈[2,16]），修正标准 LoRA 在大宽度网络下的次优优化行为。

- **2024-03 · GaLore（梯度低秩投影）** — Zhao et al., ICML 2024. [arXiv:2403.03507](https://arxiv.org/abs/2403.03507) — 对梯度做低秩 SVD 投影，优化器状态只在 r 维子空间维护（每 T 步刷新），全量参数均可更新，大幅降低预训练/持续预训练显存需求。

- **2024-04 · PiSSA（主成分初始化适配）** — Meng et al., NeurIPS 2024. [arXiv:2404.02948](https://arxiv.org/abs/2404.02948) — 用 W₀ 的 SVD 前 r 个主分量初始化 LoRA 的 A、B 矩阵，frozen 部分为 residual，使初始点更接近预训练权重结构，收敛速度快于标准 LoRA。
